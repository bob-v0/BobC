...
struct token
{
    int type;
    int flags;
    struct pos pos;                    // position of the token in the source file
    union
    {
        char cval;
        const char *sval;
    ...
    // Is True if there is whitespace between the token and next token
    // i.e. * a for operator token * would mean whitespace would be set for token "a"
    bool whitespace;

    // (Hello World)
    const char* between_brackets;      // points to the beginning of what is between () for debugging purposes
};
